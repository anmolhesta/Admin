...
class  CommonService
{
    ...
    {
        try {
            //  dd((!empty($this->request->password)) , $authuser->password , $authuser->getOriginal('password') );
            //$validated = $this->request->validated();
            $result = DB::transaction(function () use ($authuser) {
                $user = $authuser->update([
                    'name' => ((!empty($this->request->name))  && $authuser->name != $this->request->name) ?  $this->request->name : $authuser->name,
                    'address' => ((!empty($this->request->address))   && $authuser->address != $this->request->address ) ? $this->request->address : $authuser->address,
                    'password' => ( (!empty($this->request->password)) && $authuser->password != Hash::make($authuser->password) ) ? Hash::make($this->request->password) : $authuser->password,
                    'ref_status_id' => 2,
                ]);

                if ((!empty($this->request->file('profile_picture') && !empty($this->request->file('profile_picture')))) && $this->request->file('profile_picture') && !empty($this->request->file('profile_picture'))) {
